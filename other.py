...
if input_datetime <= today:
    print(f"Error: The entered date {input_date} should be a future date relative to today ({today.strftime('%Y-%m-%d')}). Please enter a future date.")
else:
    # Calculate the past 5 days for analysis relative to the input date
    print(f"Analyzing past traffic data for the last 5 days relative to {input_date} at {input_hour}:00.")
    ...
    for i in range(1, 6):  # Check the last 5 days relative to the input date
        ...
        # Ensure the past date falls within valid days (before today)
        if past_date.date() > today.date():
            continue  # Skip days that are beyond today
    ...
    if len(past_counts) >= 5:  # Ensure we have at least 5 days of data
        ...
        print(f"Not enough historical data for the last 5 days relative to {input_date}. Cannot make a prediction.")
